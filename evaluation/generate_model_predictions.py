...
OUTPUT_FINAL_CSV = "results/22_03_run_test_80k_EC0051/my_test_data_100.csv"
...
MODELS = {
    ...
    "Distilled": {
        "config_name": "Qwen3-0.6B-MoE",
        "path": "checkpoints/Qwen/Qwen3-0.6B-Agri-Distilled/22_03_run_test_80k_EC0051/step-11999.pth"
    },
...
# ==========================================================
# SPLIT THOUGHT AND ADVISORY
# ==========================================================

def split_thought_advisory(text):

    # detect first Hindi character
    hindi_pattern = re.compile(r'[\u0900-\u097F]')

    match = hindi_pattern.search(text)

    if match:
        split_pos = match.start()

        thought = text[:split_pos].strip()
        advisory = text[split_pos:].strip()

        if len(thought) < 5:
            thought = "NO_THOUGHT_GENERATED"

        return thought, advisory

    return "NO_THOUGHT_GENERATED", text.strip()
...
def main():
    ...
    for model_key, config_data in MODELS.items():
        ...
        for _, row in tqdm(df_eval.iterrows(), total=len(df_eval)):
            ...
            if len(input_ids) > MAX_CONTEXT - 300:
                input_ids = input_ids[:MAX_CONTEXT - 300]
            ...
            thought_text, advisory_text = split_thought_advisory(raw_output)
