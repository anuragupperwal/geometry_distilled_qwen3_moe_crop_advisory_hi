...
# # --- CONFIGURATION ---
# log_file = "outputs/20_03_run_test_80k_EC0051/training_log.csv"
...
def plot_training_metrics():
    ...
    if 'total_loss' in df.columns:
        ...
        sns.lineplot(data=df, x="step", y="total_loss_smooth", linewidth=1.5)
        plt.title("Total Loss Convergence", fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("Loss")
        plt.tight_layout()
        plt.savefig(output_dir / "loss_total.png", dpi=300)
        plt.close()
    ...
    if 'kl_loss' in df.columns:
        ...
        sns.lineplot(data=df, x="step", y="kl_loss_smooth", linewidth=1.5, color="orange")
        plt.title("KL Distillation Loss", fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("Loss")
        plt.tight_layout()
        plt.savefig(output_dir / "loss_kl.png", dpi=300)
        plt.close()
    ...
    if 'ce_loss' in df.columns:
        plt.figure(figsize=(14, 6))
        sns.lineplot(data=df, x="step", y="ce_loss_smooth", linewidth=1.5, color="green")
        plt.title("Cross-Entropy Loss", fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("Loss")
        plt.tight_layout()
        plt.savefig(output_dir / "loss_ce.png", dpi=300)
        plt.close()
    ...
    if 'cka_loss' in df.columns:
        plt.figure(figsize=(14, 6))
        sns.lineplot(data=df, x="step", y="cka_loss_smooth", linewidth=1.5, color="purple")
        plt.title("CKA Feature Alignment (Smoothed)", fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("CKA Distance")
        plt.tight_layout()
        plt.savefig(output_dir / "cka_alignment.png", dpi=300)
        plt.close()
    ...
    if 'max_load' in df.columns:
        plt.figure(figsize=(14, 6))
        sns.lineplot(data=df, x="step", y="max_load_smooth", linewidth=1.5, color="red")

        plt.axhline(y=0.125, linestyle="--", color="black", label="Ideal (12.5%)")

        plt.title("Max Expert Load Over Time", fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("Load")
        plt.legend()
        plt.tight_layout()
        plt.savefig(output_dir / "moe_max_load.png", dpi=300)
        plt.close()
    ...
    if 'div_loss' in df.columns:
        plt.figure(figsize=(14, 6))
        sns.lineplot(data=df, x="step", y="div_loss_smooth", linewidth=1.5, color="gray")

        plt.title("Expert Diversity Loss", fontweight='bold')
        plt.xlabel("Steps")
        plt.ylabel("Loss")
        plt.tight_layout()
        plt.savefig(output_dir / "moe_diversity.png", dpi=300)
        plt.close()
