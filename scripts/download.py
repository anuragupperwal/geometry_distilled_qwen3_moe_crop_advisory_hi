...
def download_from_hub(
    ...
    previous = getattr(constants, "HF_HUB_ENABLE_HF_TRANSFER", False)
    if _HF_TRANSFER_AVAILABLE and not previous:
        ...
        if hasattr(download, "HF_HUB_ENABLE_HF_TRANSFER"):
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Download weights from Hugging Face Hub.")
    parser.add_argument("--repo_id", type=str, required=True, help="The HF repo ID (e.g., Qwen/Qwen3-0.6B)")
    parser.add_argument("--tokenizer_only", action="store_true", help="Download only tokenizer files")
    parser.add_argument("--model_name", type=str, default=None, help="The config name if different from repo_id")
    parser.add_argument("--checkpoint_dir", type=Path, default=Path("checkpoints"), help="Directory to save weights")
    ...
    download_from_hub(
        ...
        convert_checkpoint=True  # This triggers the automatic LitGPT conversion
    )
